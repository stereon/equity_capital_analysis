...
| Agent 策略问股 | 多轮追问，支持均线、缠论、波浪、趋势、热点、事件、成长、预期等 15 种内置策略，覆盖 Bot/API |
...
> 功能细节、字段契约、基本面 P0 超时语义、交易纪律、数据源优先级、API 行为请看 [完整配置与部署指南](docs/full-guide.md)。
...
> Docker 部署、定时任务、云服务器访问请参考 [完整指南](docs/full-guide.md)。
...
需要通过 HTTP 调用分析能力时，启动 FastAPI 服务：

```bash
python main.py --serve         # 同时执行分析任务
python main.py --serve-only    # 仅启动 API，不自动分析
```

默认监听 `http://127.0.0.1:8000`，通过 `--host`/`--port` 可调整。鉴权与端点细节见 [完整指南](docs/full-guide.md)。

## 🤖 Agent 策略问股

配置任意可用 AI API Key 后即可通过 API 或机器人渠道使用策略问股；如需显式关闭可设置 `AGENT_MODE=false`。
...
> Agent 具体参数、`skill` 命名兼容、多 Agent 模式和预算护栏见 [完整指南](docs/full-guide.md) 与 [LLM 配置指南](docs/LLM_CONFIG_GUIDE.md)。
