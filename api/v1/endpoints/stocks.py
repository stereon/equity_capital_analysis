...
def _cache_get(store: Dict[str, Tuple[float, Any]], key: str, ttl: int):
    with _cache_lock:
        item = store.get(key)
    if not item:
        return None
...
def get_stock_quote(stock_code: str) -> StockQuote:
    ...
    """
    try:
        cache_key = stock_code.strip().upper()
        cached: Optional[StockQuote] = _cache_get(_quote_cache, cache_key, _QUOTE_TTL_SECONDS)
...
def get_stock_history(
    ...
    """
    try:
        cache_key = f"{stock_code.strip().upper()}|{period}|{days}"
        cached: Optional[StockHistoryResponse] = _cache_get(
            _history_cache, cache_key, _HISTORY_TTL_SECONDS,
        )
        if cached is not None:
            return cached
        ...
        _cache_set(_history_cache, cache_key, response)
        return response
