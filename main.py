...
示例:
  ...
  python main.py --market-review    # 仅运行大盘复盘
    ...
    parser.add_argument(
        '--recommend',
        action='store_true',
        help='基于今日热门板块和技术评分输出候选股推荐(不调用 LLM)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='--recommend 模式下输出的候选股数量(默认 10)'
    )
    ...
    parser.add_argument(
        '--morning-briefing',
        action='store_true',
        help='生成早盘播报(隔夜美股 + 港股早盘 + 新闻 + LLM 策略提示)'
    )

    parser.add_argument(
        '--closing-summary',
        action='store_true',
        help='生成收盘总结(A 股 + 港股 + 板块 + 新闻 + LLM 复盘,港股收盘后跑)'
    )
...
def main() -> int:
    ...
    try:
        ...
        if getattr(args, 'morning_briefing', False):
            logger.info("模式: 早盘播报")
            from src.services.briefing_service import MorningBriefingService

            result = MorningBriefingService(config).generate(
                send_notification=not getattr(args, 'no_notify', False),
            )
            logger.info(f"早盘播报完成: record_id={result.get('record_id')}")
            return 0

        # 模式: 收盘总结
        if getattr(args, 'closing_summary', False):
            logger.info("模式: 收盘总结")
            from src.services.briefing_service import ClosingSummaryService

            result = ClosingSummaryService(config).generate(
                send_notification=not getattr(args, 'no_notify', False),
            )
            logger.info(f"收盘总结完成: record_id={result.get('record_id')}")
            return 0

        # 模式: 候选股推荐(技术筛选,不跑 LLM)
        if getattr(args, 'recommend', False):
            logger.info("模式: 候选股推荐")
            from src.services.recommendation_service import recommend

            watchlist = list(config.stock_list or []) if getattr(config, 'stock_list', None) else []
            result = recommend(
                top_n=getattr(args, 'recommend_top', 10),
                pool=getattr(args, 'recommend_pool', 'hs300'),
                watchlist=watchlist,
            )
            top = result.get('candidates') or []
            sectors = result.get('hot_sectors') or []
            logger.info(f"今日领涨板块: {sectors}")
            logger.info(f"===== 候选股 Top {len(top)} =====")
            for i, c in enumerate(top, 1):
                signals = " / ".join(c.get('signals') or []) or '-'
                logger.info(
                    f"{i:>2}. {c.get('code')} {c.get('name','')} "
                    f"[{c.get('industry') or '-'}] 评分 {c.get('score')} | "
                    f"现价 {c.get('last_price'):.2f} ({c.get('change_pct'):+.2f}%) | {signals}"
                )
            if result.get('report_path'):
                logger.info(f"详细报告: {result['report_path']}")
            return 0
