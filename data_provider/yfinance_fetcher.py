...
class YfinanceFetcher(BaseFetcher):
    ...
    def _normalize_data(self, df: pd.DataFrame, stock_code: str) -> pd.DataFrame:
        ...
        # 新版 yfinance + multi_level_index=True 返回的 DataFrame 索引无 name，
        # 直接 reset_index 会生成 'index' 列而非 'Date'，下面的列映射会漏掉日期列。
    ...
    def _get_us_stock_quote_from_stooq(self, stock_code: str) -> Optional[UnifiedRealtimeQuote]:
        ...
        request = Request(
            url,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; DSA/1.0; +https://github.com/stereon/equity_capital_analysis)",
