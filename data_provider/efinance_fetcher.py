...
# eastmoney slist 接口 f19 字段 -> 板块类型
_BOARD_TYPE_MAP = {1: "地域", 2: "行业", 3: "概念"}
...
class EfinanceFetcher(BaseFetcher):
    ...
    def _fetch_belong_board_with_type(self, stock_code: str) -> Optional[pd.DataFrame]:
        """
        通过 eastmoney slist 接口获取所属板块，并解析 f19 字段映射板块类型。

        相比 ef.stock.get_belong_board 复用同一可达端点（slist），但额外请求 f19
        （1=地域 / 2=行业 / 3=概念），输出附带「板块类型」列。
        """
        from efinance.stock.getter import session, get_quote_id

        secid = get_quote_id(stock_code, suppress_error=True)
        if not secid:
            return None
        ...
        for item in rows:
            ...
            try:
                if raw_change not in (None, "-", ""):
                    change_pct = float(raw_change) / 100
            ...
            records.append({
                ...
                "板块名称": str(name).strip(),
                "板块涨幅": change_pct,
                "板块类型": _BOARD_TYPE_MAP.get(item.get("f19")),
            })
    ...
    def get_belong_board(self, stock_code: str) -> Optional[pd.DataFrame]:
        """
        获取股票所属板块（含板块类型：行业/概念/地域）

        优先走带 f19 的 slist 请求解析板块类型；若失败则回退到
        ef.stock.get_belong_board（板块名/代码仍可用，类型为空）。
        ...
        try:
            ...
            logger.info(f"[API调用] 获取 {stock_code} 所属板块(含类型)...")
            api_start = time.time()

            df = _ef_call_with_timeout(self._fetch_belong_board_with_type, stock_code)
            if df is None or df.empty:
                # 回退：仅板块名/代码，无类型
            ...
            if df is not None and not df.empty:
                logger.info(f"[API返回] 获取所属板块成功: 返回 {len(df)} 个板块, 耗时 {api_elapsed:.2f}s")
                return df
            else:
            ...
            logger.warning(f"[超时] 获取 {stock_code} 所属板块超过 {_EF_CALL_TIMEOUT}s，跳过")
            return None
        except Exception as e:
