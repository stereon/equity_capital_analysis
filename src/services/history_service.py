...
class HistoryService:
    ...
    def get_markdown_report(self, record_id: str) -> Optional[str]:
        ...
        report_type_val = getattr(record, "report_type", None)
        if report_type_val == "market_review":
        ...
        # 自定义报告(早盘 / 收盘 / 荐股 等)的 markdown 已经是完整模板,
        # 不走个股拼装,直接返回 raw_response。
        if report_type_val in {"morning_briefing", "closing_summary", "recommendation"}:
            if isinstance(raw_result, dict):
                content = raw_result.get("raw_response")
                if isinstance(content, str) and content.strip():
                    return content
            news_content = getattr(record, "news_content", None)
            if isinstance(news_content, str) and news_content.strip():
                return news_content
            raise MarkdownReportGenerationError(
                f"{report_type_val} report has no markdown content for record {record_id}",
                record_id=record_id,
            )
