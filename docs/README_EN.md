...
> Docker deployment, scheduling, and cloud-server access are documented in the [Full Guide](./full-guide_EN.md).
...
To invoke analysis capabilities over HTTP, start the FastAPI service:

```bash
python main.py --serve         # run scheduled analysis alongside the API
python main.py --serve-only    # API only, no automatic analysis
```

The service listens on `http://127.0.0.1:8000` by default; override with `--host`/`--port`. Authentication and endpoint details are in the [Full Guide](./full-guide_EN.md).

## 🤖 Agent Strategy Chat

After configuring any available AI API key, the strategy chat is accessible via the API or bot channels. Set `AGENT_MODE=false` only if you want to disable it explicitly.
...
> Agent parameters, `skill` naming compatibility, multi-agent mode, and budget guards are covered in the [Full Guide](./full-guide_EN.md) and [LLM Config Guide](./LLM_CONFIG_GUIDE_EN.md).
