...
- 当前仓库对此链路的依赖约束是 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`（见 `requirements.txt`）；回归覆盖包括 `tests/test_system_config_service.py` 与 `tests/test_system_config_api.py`。
...
- 建议回退操作链路：先导出 `.env` 备份，通过 `POST /api/v1/system/config/import` 导入后，再通过 `GET /api/v1/system/config` 刷新配置，确认 `LITELLM_MODEL / AGENT_LITELLM_MODEL / VISION_MODEL / LLM_TEMPERATURE` 与模型列表一致后再继续使用。
...
- 旧配置回退路径：手动恢复 `.env` 中的 `LLM_* / LITELLM_* / AGENT_LITELLM_MODEL / VISION_MODEL / LLM_TEMPERATURE`，或通过 `POST /api/v1/system/config/import` 导入备份；API 导入前需开启鉴权（`ADMIN_AUTH_ENABLED=true`），否则会返回 403。
