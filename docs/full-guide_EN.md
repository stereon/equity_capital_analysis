...
| `SERPAPI_API_KEYS` | [SerpAPI](https://serpapi.com/baidu-search-api?utm_source=github_equity_capital_analysis) search-engine results for realtime financial news | Recommended |
...
The Dockerfile builds the backend image. The runtime invokes `python main.py` to run CLI tasks or the FastAPI service.

Official image registries:

- GHCR: `ghcr.io/stereon/equity_capital_analysis:<tag>`
- Docker Hub: `<DOCKERHUB_USERNAME>/equity_capital_analysis:<tag>` (driven by the publisher's `DOCKERHUB_USERNAME` secret; the official release uses `stereon/equity_capital_analysis`)

### Quick Start

```bash
# 1. Clone repository
git clone https://github.com/stereon/equity_capital_analysis.git
...
# 3. Start container
docker-compose -f ./docker/docker-compose.yml up -d server     # API service mode (provides FastAPI HTTP)
...
# 4. Access API
# http://localhost:8000  (visit /docs for Swagger UI)
...
```bash
# Web/API mode
docker pull stereon/equity_capital_analysis:latest
docker run -d \
  --name dsa-server \
  --env-file .env \
  -p 8000:8000 \
  ...
  stereon/equity_capital_analysis:latest \
...
|------|------|------|
| `docker-compose -f ./docker/docker-compose.yml up -d server` | API service mode, provides FastAPI HTTP | 8000 |
...
>
> When the built-in scheduler is started via `python main.py --schedule`, `python main.py --serve --schedule`, or an equivalent local mode, saving a new `SCHEDULE_TIME` via the API will rebind the daily job on the next scheduler poll without restarting the process. The previous trigger time is removed instead of being kept alongside the new one.
...
Run the FastAPI server to drive configuration, analysis triggering, task tracking, history reports, and backtesting over HTTP/REST. Authentication and API usage details are below.
...
> Note: when `/api/v1/analysis/market-review` returns a `task_id`, callers should poll `GET /api/v1/analysis/status/{task_id}` to track `pending/processing` progress; status `completed` indicates the report has been generated and pushed, status `failed` returns the failure reason via the `error` field.
...
> - Rollback path: if regression appears, restore historical `LITELLM_MODEL`, `LITELLM_FALLBACK_MODELS`, and legacy `GEMINI_*` / `OPENAI_*` / `ANTHROPIC_*` / `DEEPSEEK_*`, or import a backup through `POST /api/v1/system/config/import` (after enabling admin auth) and restart; at runtime you can also clear `LITELLM_CONFIG` / `LLM_CHANNELS` to force legacy fallback.
