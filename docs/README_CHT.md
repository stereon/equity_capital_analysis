...
| Agent 策略問股 | 多輪追問，支援均線、纏論、波浪、趨勢、熱點、事件、成長、預期等 15 種內建策略，覆蓋 Bot/API |
...
> Docker 部署、定時任務、雲端伺服器訪問請參考 [完整指南](./full-guide.md)。
...
需要透過 HTTP 呼叫分析能力時，啟動 FastAPI 服務：

```bash
python main.py --serve         # 同時執行分析任務
python main.py --serve-only    # 僅啟動 API，不自動分析
```

預設監聽 `http://127.0.0.1:8000`，透過 `--host`/`--port` 可調整。鑑權與端點細節見 [完整指南](./full-guide.md)。

## 🤖 Agent 策略問股

配置任意可用 AI API Key 後即可透過 API 或機器人渠道使用策略問股；如需顯式關閉可設定 `AGENT_MODE=false`。
...
> Agent 具體參數、`skill` 命名兼容、多 Agent 模式和預算護欄見 [完整指南](./full-guide.md) 與 [LLM 配置指南](./LLM_CONFIG_GUIDE.md)。
