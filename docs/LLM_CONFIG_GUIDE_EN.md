...
- The current dependency constraint for this flow in the repository is `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0` (see `requirements.txt`). Regression coverage for it lives in `tests/test_system_config_service.py` and `tests/test_system_config_api.py`.
...
- Recommended rollback sequence: export `.env` backup, restore via `POST /api/v1/system/config/import`, then call `GET /api/v1/system/config` to refresh the runtime config and verify `LITELLM_MODEL` / `AGENT_LITELLM_MODEL` / `VISION_MODEL` / `LLM_TEMPERATURE` before continuing.
