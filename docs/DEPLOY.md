...
启动成功后，FastAPI 服务监听 `http://服务器公网IP:8000`，可通过 `/docs` 查看 OpenAPI 文档；要外网访问记得先在云服务器控制台的「安全组」里放行 8000 端口。
...
# 启动 FastAPI 服务（云服务器外网访问需用 --host 0.0.0.0 并放行端口）
python main.py --serve-only

# 启动 FastAPI 服务并执行一次分析（需每日定时请加 --schedule 或设 SCHEDULE_ENABLED=true）
...
# 创建 GitHub 仓库并推送
# 在 GitHub 网页上创建新仓库后：
git remote add origin https://github.com/你的用户名/equity_capital_analysis.git
git branch -M main
